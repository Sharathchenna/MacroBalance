...
Future<String> processImageWithGemini(String imagePath) async {
  try {
    print(
        '[Gemini Debug] Starting Gemini processing at ${DateTime.now().toString()}');
    ...
    print(
        '[Gemini Debug] Model initialized in ${DateTime.now().difference(startTime).inMilliseconds}ms');

    final promptText = '''
    Analyze the following meal and provide its nutritional content. 
    If the image is a nutrition label, extract the nutrition information from the label and return response in the specified format, give correct nutrition values for different serving sizes, if given percentages then convert them into relevant values, do not provide null values if you couldn't get the number return 0.
    ...
    IMPORTANT: Return ONLY a valid JSON object. Do not include markdown formatting, explanations, or any other text. Do not use ```json or ``` markers.
    ...
    print(
        '[Gemini Debug] Gemini API response received in ${DateTime.now().difference(apiCallStart).inMilliseconds}ms');
    ...
    print(
        '[Gemini Debug] Total Gemini processing time: ${DateTime.now().difference(startTime).inMilliseconds}ms');
