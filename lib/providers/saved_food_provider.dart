...
class SavedFoodProvider with ChangeNotifier {
  ...
  SavedFoodProvider() {
    // Simplified initialization without connectivity monitoring
  }
  ...
  Future<void> initialize() async {
    ...
    print('SavedFoodProvider: Starting initialization...');
    _isLoading = true;
    ...
    try {
      ...
      _loadFromCache();
      print('SavedFoodProvider: After cache load, have ${_savedFoods.length} foods');

      // Then try to load from local storage
      final localFoods = await _repository.loadFromLocal();
      print('SavedFoodProvider: Loaded ${localFoods.length} foods from local storage');
      _updateFoodsList(localFoods);
      print('SavedFoodProvider: After local load, have ${_savedFoods.length} foods');

      // Assume network connection is available (simplified)
      _hasNetworkConnection = true;

      // Start background sync if network is available
      if (_hasNetworkConnection) {
        print('SavedFoodProvider: Starting background sync...');
        await _startBackgroundSync();
        print('SavedFoodProvider: After sync, have ${_savedFoods.length} foods');
      }

      _isInitialized = true;
      print('SavedFoodProvider: Initialization complete. Final count: ${_savedFoods.length}');
    } catch (e) {
      print('SavedFoodProvider: Failed to initialize saved foods: $e');
  ...
  Future<void> addSavedFood(dynamic food) async {
    print('SavedFoodProvider: Adding food to saved foods...');
    print('Food type: ${food.runtimeType}');
    if (food is search.FoodItem) {
      print('Food name: ${food.name}, ID: ${food.fdcId}');
    }
    ...
    print('SavedFoodProvider: Food added successfully. Total saved foods: ${_savedFoods.length}');
    _safeNotifyListeners();
  ...
  bool isFoodSaved(String foodId) {
    ...
    print('SavedFoodProvider: Checking if food $foodId is saved: $isSaved');
    print('SavedFoodProvider: Currently have ${_savedFoods.length} saved foods');
    return isSaved;
  }
