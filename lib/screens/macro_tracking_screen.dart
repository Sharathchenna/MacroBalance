...
class _MacroTrackingScreenState extends State<MacroTrackingScreen>
  ...
  Future<void> _loadInitialData() async {
    ...
    try {
      ...
      for (int i = 29; i >= 0; i--) {
        ...
        history.add(
            {'date': date, 'protein': protein, 'carbs': carbs, 'fat': fat});
      }
  ...
  void _updateMacroPercentages(
      double totalCalories, Map<String, double> nutrientTotals) {
    final percentages = {
      'protein': totalCalories > 0
          ? ((nutrientTotals['protein'] ?? 0.0) * 4 / totalCalories * 100)
          : 0,
      'carbs': totalCalories > 0
          ? ((nutrientTotals['carbs'] ?? 0.0) * 4 / totalCalories * 100)
          : 0,
      'fat': totalCalories > 0
          ? ((nutrientTotals['fat'] ?? 0.0) * 9 / totalCalories * 100)
          : 0,
    };
              ...
              child: Consumer<FoodEntryProvider>(
                builder: (context, foodEntryProvider, child) {
                  ...
                  final nutrientTotals = foodEntryProvider
                      .getNutritionTotalsForDate(DateTime.now());
        ...
        children: [
          ...
          Container(
            ...
            decoration: BoxDecoration(
              ...
              borderRadius: BorderRadius.circular(20),
              border: Border.all(
                color: customColors.accentPrimary.withValues(alpha: 0.2),
                width: 1.5,
              ),
            ),
            child: Row(
                                ...
                                onPressed: () async {
                                  ...
                                  final foodEntryProvider =
                                      Provider.of<FoodEntryProvider>(context,
                                          listen: false);
                                  // Use updateNutritionGoals instead of individual setters
                                  await foodEntryProvider.updateNutritionGoals(
                                    NutritionGoals(
                                      ...
                                      carbs: carbs,
                                      fat: fat,
                                      steps: foodEntryProvider.stepsGoal,
                                      bmr: foodEntryProvider.bmr,
                                      tdee: foodEntryProvider.tdee,
                                      goalWeightKg:
                                          foodEntryProvider.goalWeightKg,
                                      currentWeightKg:
                                          foodEntryProvider.currentWeightKg,
                                      goalType: foodEntryProvider.goalType,
                                      deficitSurplus:
                                          foodEntryProvider.deficitSurplus,
                                    ),
                                  );
                                ...
                                onPressed: () async {
                                  try {
                                    ...
                                    await foodEntryProvider
                                      ...
                                      NutritionGoals(
                                        calories: calculateDialogCalories(),
                                        protein: protein,
                                        carbs: carbs,
                                        fat: fat,
                                        steps: foodEntryProvider.stepsGoal,
...
class _MacroBarChartPainter extends CustomPainter {
  ...
  void _drawYAxis(Canvas canvas, Rect chartArea) {
    final paint = Paint()
      ..color = customColors.textSecondary.withValues(alpha: 0.2)
      ..strokeWidth = 1;
