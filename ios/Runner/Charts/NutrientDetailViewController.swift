            ...
            if entry.meals.isEmpty {
                ...
                HStack {
                    VStack(spacing: 8) {
                        ...
                        ZStack {
                            ForEach(0..<entry.meals.count, id: \.self) { index in
                                let meal = entry.meals[index]
                                let startAngle = index == 0 ? 0.0 : 
                                    entry.meals[0..<index].reduce(0) { $0 + $1.calories } / entry.calories
                                let endAngle = entry.meals[0...index].reduce(0) { $0 + $1.calories } / entry.calories
